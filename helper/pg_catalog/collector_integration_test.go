...
func collectAndVerify(t *testing.T, db *sql.DB) {
	...
	if serverVersionNum >= 180000 {
		...
		// A VIRTUAL generated column is 18-only too, and is the other half of attgenerated.
	...
	t.Run("generated columns record how they are stored", func(t *testing.T) {
		computed, ok := findRelation(preview, "computed")
		require.True(t, ok)

		base, ok := findColumn(preview, computed.OID, "base")
		require.True(t, ok)
		assert.Empty(t, base.Generated, "a plain column is generated by nothing")

		stored, ok := findColumn(preview, computed.OID, "stored_total")
		require.True(t, ok)
		assert.Equal(t, "s", stored.Generated)

		// VIRTUAL is 18-only, and is what makes SET EXPRESSION a metadata change ⟨R-AT-SETEXPR⟩.
		virtual, ok := findColumn(preview, computed.OID, "virtual_total")
		if version < pg_contract.Version18 {
			assert.False(t, ok, "a virtual generated column cannot exist before 18")
			return
		}
		require.True(t, ok)
		assert.Equal(t, "v", virtual.Generated)
	})
