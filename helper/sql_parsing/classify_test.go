...
func classify(sql string, version float32) metricshelper.EventOperationType {
...
func TestClassifyOperation(t *testing.T) {
	...
	cases := []struct {
		...
		{"add col default now()", "ALTER TABLE t ADD COLUMN a timestamptz DEFAULT now()", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		{"add col default gen_random_uuid", "ALTER TABLE t ADD COLUMN a uuid DEFAULT gen_random_uuid()", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		{"add col generated stored", "ALTER TABLE t ADD COLUMN a int GENERATED ALWAYS AS (b + 1) STORED", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		{"add col generated identity", "ALTER TABLE t ADD COLUMN a int GENERATED ALWAYS AS IDENTITY", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		...
		{"type varchar to text", "ALTER TABLE t ALTER COLUMN a TYPE text", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		{"type varchar widen to text", "ALTER TABLE t ALTER COLUMN a TYPE text USING a::text", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		{"type to int rewrite", "ALTER TABLE t ALTER COLUMN a TYPE bigint", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		...
		{"varchar to text coercible", "ALTER TABLE t ALTER COLUMN a TYPE text /* from varchar */", pg15, metricshelper.EventOperationTypeRewriteUnderLock},

		// --- Constraints ---
		{"add check", "ALTER TABLE t ADD CONSTRAINT c CHECK (a > 0)", pg15, metricshelper.EventOperationTypeScanUnderLock},
		{"add check not valid", "ALTER TABLE t ADD CONSTRAINT c CHECK (a > 0) NOT VALID", pg15, metricshelper.EventOperationTypeMetadataOnly},
		{"add fk", "ALTER TABLE t ADD CONSTRAINT c FOREIGN KEY (a) REFERENCES u (id)", pg15, metricshelper.EventOperationTypeScanUnderLock},
		...
		{"add unique", "ALTER TABLE t ADD CONSTRAINT c UNIQUE (a)", pg15, metricshelper.EventOperationTypeScanUnderLock},
		{"add primary key", "ALTER TABLE t ADD CONSTRAINT c PRIMARY KEY (a)", pg15, metricshelper.EventOperationTypeScanUnderLock},
		...
		// --- Indexes ---
		{"create index", "CREATE INDEX idx ON t (a)", pg15, metricshelper.EventOperationTypeScanUnderLock},
		{"create index concurrently", "CREATE INDEX CONCURRENTLY idx ON t (a)", pg15, metricshelper.EventOperationTypeNonBlocking},
		{"drop index", "DROP INDEX idx", pg15, metricshelper.EventOperationTypeMetadataOnly},
		{"drop index concurrently", "DROP INDEX CONCURRENTLY idx", pg15, metricshelper.EventOperationTypeNonBlocking},
		{"reindex", "REINDEX INDEX idx", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		...
		{"refresh matview", "REFRESH MATERIALIZED VIEW mv", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
		...
		{"set default + type change", "ALTER TABLE t ALTER COLUMN a SET DEFAULT 1, ALTER COLUMN b TYPE bigint", pg15, metricshelper.EventOperationTypeRewriteUnderLock},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := classify(tc.sql, tc.version)
			if got != tc.want {
