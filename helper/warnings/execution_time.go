...
func GenerateExecutionTimeWarnings(preMigrationRun *output.OutputDatabaseMigrationExplainQueryRun, postMigrationRun *output.OutputDatabaseMigrationExplainQueryRun, file string, query string) string {
	...
	if bPlannedTimeRegression && bRealTimeRegression {
		warningText = fmt.Sprintf("Query execution time regression exceeded %d%% threshold\nfile: %s\nquery: %s", ExecutionTimeThreshold*100, file, query)
	} else {
		if bRealTimeRegression {
			warningText = fmt.Sprintf("Query real execution time regression exceeded %d%% threshold\nfile: %s\nquery: %s", ExecutionTimeThreshold*100, file, query)
		}
		if bPlannedTimeRegression {
			warningText = fmt.Sprintf("Query planned execution time regression exceeded %d%% threshold\nfile: %s\nquery: %s", ExecutionTimeThreshold*100, file, query)
		}
	}
	return warningText
}
